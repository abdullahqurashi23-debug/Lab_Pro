// Types mirroring the schema in migrations/001_init.sql. These are the main
// process's own copy — the renderer has its own equivalents under
// src/lib/types.ts once the UI is updated to match (see the note in
...
export interface TestParameter {
  ...
  dropdown_options: string; // JSON-encoded string[]
  formula: string;
...
// insert as new. This is what lets updateTest() edit a test's parameters
// without ever deleting a row that report_results.parameter_id points at.
...
export interface ReportWithDetails extends Report {
  ...
  tests: ReportTest[];
  payments: Payment[];
  // total - paid - SUM(payments.amount) — the amount still owed AFTER
  // accounting for every payment ever recorded against this report,
  // whether collected at creation or added later via the payments ledger.
...
export interface NewReportInput {
  patient: {
    id?: number; // reuse an existing patient instead of creating a new one
    full_name: string;
    age: number | null;
    age_unit: AgeUnit;
    gender: Gender | null;
