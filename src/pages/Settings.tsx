  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground">
        Pick any PDF and LabPro will re-compute its SHA-256 hash and check it against every finalized report's stored
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            ...
            will be lost. A safety backup of the current database is taken first, and LabPro will restart immediately
            afterward.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <div className="space-y-1.5">
    ...
    <div className="space-y-4">
      <div className="space-y-1.5">
        ...
        <p className="text-xs text-muted-foreground">
          ...
          every app close, keeping the last 30. Leave blank to use the default (Documents/LabPro Backups).
        </p>
        <div className="flex items-center gap-2">
          <Input value={folder} onChange={(e) => setFolder(e.target.value)} placeholder="Documents/LabPro Backups (default)" className="flex-1" />
          <Button
            type="button"
            variant="outline"
            onClick={async () => {
...
function DatabaseHealthSection() {
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-muted-foreground">
        LabPro also runs this automatically every time it starts, and shows a warning immediately if a problem is found.
      </p>
  ...
  return (
    <div className="p-8 space-y-6 max-w-4xl mx-auto">
      ...
      {isAdmin && (
        <SectionCard
          title="Print Layout"
          description="Match LabPro's printed output to your pre-printed letterhead paper."
        >
          <PrintLayoutSection />
        </SectionCard>
      )}
      ...
      {isAdmin && (
        <SectionCard title="Session" description="How long LabPro waits before locking an idle session.">
          <IdleTimeoutSection />
        </SectionCard>
      )}
      {isAdmin && (
